...
class IdentityBlock(tf.keras.layers.Layer):
  ...
  def build_conv2(self):
    return tf.keras.layers.Conv2D(self.filter2,
                                ...
                                kernel_regularizer=tf.compat.v1.keras.utils.get_or_create_layer("regularizer2", self.build_regularizer),
                                kernel_initializer=self.initializer, use_bias=False)
  ...
  def build_conv3(self):
    return tf.keras.layers.Conv2D(self.filter3,
                                ...
                                kernel_regularizer=tf.compat.v1.keras.utils.get_or_create_layer("regularizer3", self.build_regularizer),
                                kernel_initializer=self.initializer, use_bias=False)
  ...
  def call(self, X_input, kernel_size, filters, stage, block, is_relu=False):
    ...
    with tf.compat.v1.variable_scope("id_block_stage" + str(self.stage) + self.block):
      ...
      x = tf.compat.v1.keras.utils.get_or_create_layer("conv1", self.build_conv1)(X_input)
      x = tf.compat.v1.keras.utils.get_or_create_layer("nomalizer1", self.build_normalizer)(x)
      x = activation_fn(x)

      # Second component of main path
      x = tf.compat.v1.keras.utils.get_or_create_layer("conv2", self.build_conv2)(x)
      x = tf.compat.v1.keras.utils.get_or_create_layer("nomalizer2", self.build_normalizer)(x)
      x = activation_fn(x)

      # Third component of main path
      x = tf.compat.v1.keras.utils.get_or_create_layer("conv3", self.build_conv3)(x)
      x = tf.compat.v1.keras.utils.get_or_create_layer("nomalizer3", self.build_normalizer)(x)
